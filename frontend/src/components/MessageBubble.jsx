    ...
    <div className={`flex max-w-[75%] ${isOwn ? 'flex-row-reverse' : 'flex-row'} items-end gap-2 relative`}>
      ...
      <div className={`relative px-4 py-2.5 rounded-2xl shadow-sm ${isOwn ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-slate-800 text-slate-100 rounded-bl-sm border border-slate-700'}`}>
        ...
        {message.messageType === 'IMAGE' && message.fileUrl ? (
          <div className="mb-1">
            <img src={`http://https://chat-web-1-b3uj.onrender.com${message.fileUrl}`} alt="attachment" className="max-w-full rounded-lg max-h-64 object-contain bg-black/10 shadow-sm" />
          </div>
        ) : message.messageType === 'FILE' && message.fileUrl ? (
          <div className="mb-1">
            <a href={`http://https://chat-web-1-b3uj.onrender.com${message.fileUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center p-3 bg-black/10 rounded-lg hover:bg-black/20 transition-colors border border-white/10">
